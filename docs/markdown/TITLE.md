...
この作例「Chevron7」は「Chevron1」基板を使っています。
...
に大別され、それぞれにSLG46826を1個ずつ割り当てています（[@fig:block-diagram]）。LED
...
今回はRTC「RX-8581NB[^rx8581nb]」の32.768KHz出力を受け分周する設計にしました。
...
[^would-work-but-waste]: こういう書き方は語弊があるが、実装するとなるとそれだけで１石ぶんのマクロセルを全部消費しうる
